...
# Process and filter reviews from 2025 with a valid rating
for entry in sorted_data:
    last_revision = entry.get("last_revision_at", "")
    rating = entry.get("rating", 0)
    
    # Check if the review is from 2025 and has a rating
    if last_revision.startswith("2025") and rating > 0:
